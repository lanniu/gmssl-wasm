...
int sm2_private_key_from_der(SM2_KEY *key, const uint8_t **in, size_t *inlen)
{
	...
	if(asn1_octet_string_from_der(&prikey, &prikey_len, &d, &dlen) != 1) {
		error_print();
		return -1;
	}
	// 2023-09-27 此处需要放松检查限制，否则无法适配版本2的私钥
	if(asn1_explicit_from_der(0, &params, &params_len, &d, &dlen) < 0) {
		error_print();
		return -1;
	}
	if(asn1_explicit_from_der(1, &pubkey, &pubkey_len, &d, &dlen) != 1) {
		error_print();
		return -1;
	}
	if(asn1_check(ver == EC_private_key_version) != 1) {
		error_print();
		return -1;
	}
